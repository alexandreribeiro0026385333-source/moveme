...
app.put('/motora/:id', async (req, res) => {
  ...
  if (corrida_id !== undefined) { updates.push(`corrida_id = $${paramCount}`); values.push(corrida_id); paramCount++; }
  if (passageiro_id !== undefined) { updates.push(`passageiro_id = $${paramCount}`); values.push(passageiro_id); paramCount++; }
  if (total_corridas_dia !== undefined) { updates.push(`total_corridas_dia = $${paramCount}`); values.push(total_corridas_dia); paramCount++; }
  if (total_valor_dia !== undefined) { updates.push(`total_valor_dia = $${paramCount}`); values.push(total_valor_dia); paramCount++; }
  if (latitude !== undefined) { updates.push(`latitude = $${paramCount}`); values.push(latitude); paramCount++; }
  if (longitude !== undefined) { updates.push(`longitude = $${paramCount}`); values.push(longitude); paramCount++; }
...
app.post('/corridas/:id/aceitar', async (req, res) => {
  ...
  try {
    ...
    // --- VERSÃO ANTIGA (criava nova corrida) ---
    /*
    await pool.query(`
      INSERT INTO corridas 
    ...
    */
    ...
    await pool.query(`
      UPDATE corridas 
      SET motorista_id = $1, motorista_nome = $2, status = 'em andamento'
      WHERE id = $3
    `, [motorista_id, motorista_nome, req.params.id]);
...
app.post('/chat/enviar', async (req, res) => {
  const { corrida_id, _tipo, _id, avatar, mensagem } = req.body;
  
  try {
    await pool.query(
      `INSERT INTO chat (lida, created_at, corrida_id, remetente_id, avatar, remetente_tipo, mensagem)
       VALUES (false, NOW(), $1, $2, $3, $4, $5)`,
      [corrida_id, _id, avatar, _tipo, mensagem]
    );
    res.json({ ok: true });

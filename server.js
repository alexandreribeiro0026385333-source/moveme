...
// --- ROTA ACEITAR CORRIDA ---
// Antes: criava nova linha, gerando duplicata
/*
...
// Agora: atualiza a corrida existente
app.post('/corridas/:id/aceitar', async (req, res) => {
  const { motorista_id, motorista_nome } = req.body;
...
app.get('/chat/:corrida_id', async (req, res) => {
  try {
    const result = await pool.query(
      'SELECT * FROM chat WHERE corrida_id = $1 ORDER BY criado_em ASC',
...
app.post('/chat/enviar', async (req, res) => {
  const { corrida_id, _tipo, _id, user_nome, avatar, mensagem } = req.body;
  
  try {
    await pool.query(
      'INSERT INTO chat (corrida_id, _tipo, _id, user_nome, avatar, mensagem, criado_em) VALUES ($1, $2, $3, $4, $5, $6, NOW())',
      [corrida_id, _tipo, _id, user_nome, avatar, mensagem]
    );
    res.json({ ok: true });
  } catch (err) {
    res.status(500).json({ error: err.message });
...
app.get('/chat/:corrida_id/novas/:ultima_msg_id', async (req, res) => {
  try {
    const result = await pool.query(
      'SELECT * FROM chat WHERE corrida_id = $1 AND id > $2 ORDER BY criado_em ASC',
